...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    // 获取当前子串的下个子串时，需要考虑到 new_idx 可能会和 down_idx, 因为是lower_bound
...
//             如果没写全，则将其保存起来
            if (write_byte < new_data.size()) {
                ...
                _unassemble_strs.insert(make_pair(_next_assembled_idx, move(data_to_store)));
            }
        } else {
            const string data_to_store = new_data.substr(0, new_data.size());
            _unassembled_bytes_num += data_to_store.size();
            _unassemble_strs.insert(make_pair(new_idx, move(data_to_store)));
        }
    }
